...
namespace Wcs.Application.StateMachine.Stacker
{
        ...
        IEventBus _bus) : IStateMachine
    {
        ...
        {
            bool isTranshipIn = _controller.IsTranshipPointByDeviceName(json);
            var wcsTask = default(WcsTask);
            if (isTranshipIn == true)
            {
            ...
            else
            {
                ...
                if (wcsTask == null)
                {
                    wcsTask = await GetStockOutTaskAsync(json);
                }
            ...
            if (wcsTask != null)
            {
                ...
                Serilog.Log.Logger
                    .BusinessInformation(LogCategory.Business,
                        new Common.Shared.Log.BusinessLog(json, wcsTask.SerialNumber, $"获取到堆垛机执行任务"));
            }
        }
        ...
        public async Task<WcsTask> GetStockInTaskAsync(string deviceName)
        {
            ...
            if (string.IsNullOrEmpty(taskNo))
            {
                Serilog.Log.Logger
                    .BusinessInformation(LogCategory.Business,
                        new Common.Shared.Log.BusinessLog(deviceName, -9999, "输送任务为空无法获取执行任务"));
            }

            int serialNumber = int.Parse(taskNo);
            ...
            if (wcsTasks.Any() == false)
            {
                Serilog.Log.Logger
                    .BusinessInformation(LogCategory.Business,
                        new Common.Shared.Log.BusinessLog(deviceName, serialNumber, "输送任务为空无法获取执行任务"));
            }
