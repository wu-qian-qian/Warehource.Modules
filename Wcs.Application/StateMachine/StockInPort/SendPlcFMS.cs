...
[StateAttrubite($"{nameof(DeviceTypeEnum.StockPortIn)}-{nameof(TaskExecuteStepTypeEnum.SendPlc)}")]
internal sealed class SendPlcFMS(
    IStockPortInController _controller,
    ...
    ICacheService _cacheService) : IStateMachine
{
    public async ValueTask HandlerAsync(string json, CancellationToken token = default)
    {
        ...
        else
        {
            ...
            var sendData = _controller.TryGetWritePlcTaskDataByDeviceName(json);
