...
[StateAttrubite($"{nameof(DeviceTypeEnum.StackerOutTranShip)}-{nameof(TaskExecuteStepTypeEnum.SendEndingPlc)}")]
internal class SendEndingPlcFSM(ICacheService _cacheService, IStackerTranshipOutController _controller)
    : IStateMachine
{
            ...
            cacheTask.TaskExecuteStep.TaskExecuteStepType == TaskExecuteStepTypeEnum.SendEndingPlc)
        {
            ...
            Log.Logger
                .BusinessInformation(LogCategory.Business, new BusinessLog(json,
                    ...
                    "数据写入成功，状态机更新下一状态"));
            await _cacheService.SetAsync(cacheKey, wcsTask);
        }
        else
        {
